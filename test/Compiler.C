...
TEST(Compiler, safeModeAppliesToLoadedModules) {
  hobbes::cc lc;
...
TEST(Compiler, safeModeFollowsImportsBetweenModules) {
...
TEST(Compiler, aSafePragmaCannotLiftADenialUnderSafe) {
  hobbes::cc lc;
  lc.setModuleOptions(std::vector<std::string>{"Safe"});
  bool threw = false;
  try {
...
TEST(Compiler, moduleOptionsDefaultToNoneSoEmbeddersAreUnaffected) {
